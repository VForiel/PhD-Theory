...
def refine_sources(sources_init: list[dict],
    ...
    for s in sources_init:
        p0 += [s['x_mas'], s['y_mas'], s['contrast_init']]
...
def reconstruct(data: np.ndarray,
    ...
    Orchestrates the four-step pipeline:
      ...
      Step 1 — Kernel LASSO reconstruction
          Builds and solves the linear system δd_K = A_K · x using only the
          3 Kernel outputs. The L1 penalty enforces sparsity (point sources).
          The resulting image is thresholded to detect source candidates,
          which serve as initial positions for Step 2.

      Step 2 — Parametric refinement
          Fits (x_mas, y_mas, contrast) for each source candidate simultaneously
          against all 9 Dark+Kernel outputs using weighted chi-squared
          minimisation (L-BFGS-B). The covariance weights shots noise and
          piston-induced noise, making the fit robust to atmospheric aberrations.

      Step 3 — Field rotation consistency check
          Computes the Pearson correlation between the measured residuals and
          the source model predictions across hour angles. Sources that do not
    ...
    # ------------------------------------------------------------------
    if verbose:
        print("[Step 1] Building Kernel linear system and running LASSO...")
    A_K, b_K, W_K = build_kernel_system(data, maps, f_star, cov)
    x_lasso = lasso_reconstruct(A_K, b_K, lambda_reg=lasso_lambda)
    image_lasso = x_lasso.reshape(M, M)
    if verbose:
        print(f"         LASSO done. Max contrast pixel = {image_lasso.max():.4f}")

    # ------------------------------------------------------------------
    # Source detection from LASSO image
    # ------------------------------------------------------------------
    if verbose:
        print("[Step 1] Detecting sources from LASSO image...")
        # Debug: report image statistics to help tune threshold
        pos_pixels = image_lasso[image_lasso > 0]
        if len(pos_pixels) > 0:
            print(f"         Image stats: max={image_lasso.max():.3e}, "
                  f"n_positive_pixels={len(pos_pixels)}, "
                  f"median_positive={np.median(pos_pixels):.3e}")
        else:
            print("         Warning: all LASSO pixels are zero — "
                  "try decreasing lasso_lambda.")
    sources_init = find_sources_from_image(
        image_lasso, pixel_scale, fov,
        n_sources_max=n_sources_max,
        threshold_sigma=detection_threshold_sigma
    )
    if verbose:
        print(f"         {len(sources_init)} source candidate(s) found.")
        for s in sources_init:
            print(f"         → ({s['x_mas']:.2f}, {s['y_mas']:.2f}) mas  "
                  f"contrast_init={s['contrast_init']:.3e}")

    if not sources_init:
        if verbose:
            print("         No sources detected above threshold. "
                  "Try lowering detection_threshold_sigma.")
        return {
            'f_star': f_star,
    ...
    # ------------------------------------------------------------------
    # Step 2: Parametric refinement on all Dark + Kernel outputs
    # ------------------------------------------------------------------
    if verbose:
        print("[Step 2] Parametric refinement on all Dark+Kernel outputs...")
    sources_refined = refine_sources(
        sources_init, data, maps, f_star, cov, fov
    )
    if verbose:
        for s in sources_refined:
            print(f"         Source: ({s['x_mas']:.2f}, {s['y_mas']:.2f}) mas  "
                  f"contrast={s['contrast']:.2e}  χ²_red={s['reduced_chi2']:.2f}")

    # ------------------------------------------------------------------
    # Step 3: Field rotation consistency check
    # ------------------------------------------------------------------
    if verbose:
        print("[Step 3] Field rotation consistency check...")
    sources_final = field_rotation_check(
        sources_refined, data, maps, f_star, fov
    )
    if verbose:
