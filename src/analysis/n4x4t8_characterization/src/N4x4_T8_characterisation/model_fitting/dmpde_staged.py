...
def fit_DMPDE_staged(scan_data: dict, return_metadata: bool, verbose=False):
    """
    ...
    Staged strategy
    ---------------
    First stage : optimize I = |M @ P @ Din @ E|^2 using only 1 active input configurations
    Second stage : optimize I = |Dout @ M @ P @ Din @ E|^2 with fixed Din and E from the first stage, using 2 active inputs configurations
    """

    # filter the scan data to keep only the configurations with 1 active input
    scan_data_stage1 = {}
    scan_data_stage2 = {}
    for key, value in scan_data.items():
        if len(key) == 1:  # Keep only configurations with 1 active inputs
            scan_data_stage1[key] = value
        elif len(key) == 2:  # Keep only configurations with 2 active inputs
            scan_data_stage2[key] = value
    ...
    # Downstream cross-talk matrix (spectral norm <= 1 -> allow small loss of energy)
    Dout = np.eye(4, dtype=np.complex128)
    ...
    def get_xy_space(scan_data_stage):
        """
        ...
        x_space = []
        y_space = []
        for inputs, data in scan_data_stage.items():
    ...
    x_space_stage1, y_space_stage1 = get_xy_space(scan_data_stage1)
    x_space_stage2, y_space_stage2 = get_xy_space(scan_data_stage2)

    def pack_params_stage1(Eon, Eoff, Din):
    ...
    def unpack_params_stage1(params):
    ...
    def unpack_params_stage2(params):
        D_image_mask = np.array([
            [0, 1, 1, 1],
            [0, 0, 1, 1],
            [0, 1, 0, 1],
        ...
        Dout_real = params[0:16].reshape(4, 4)
        Dout_imag = np.zeros((4, 4)) # Initialize with zeros
        Dout_imag[D_image_mask == 1] = params[16:25] # Fill only the non-diagonal and non first-input phase terms
    ...
    def spectral_penalty(matrix, target=1.0, weight=10.0, mode="equal"):
        """
        Return one residual term enforcing a spectral-norm constraint.

        Parameters
        ----------
        matrix : np.ndarray
            Complex matrix to constrain.
        target : float
            Desired spectral norm.
        weight : float
            Penalty weight.
        mode : str
            "upper" -> penalize only if norm exceeds target
            "equal" -> penalize deviation from target
        """
        sigma_max = np.linalg.svd(matrix, compute_uv=False)[0]

        if mode == "upper":
            violation = max(0.0, sigma_max - target)
        elif mode == "equal":
            violation = sigma_max - target
        else:
            raise ValueError("mode must be 'upper' or 'equal'")

        return np.sqrt(weight) * violation

    def unique_residual_stage1(x, y_true, params):

        Eon, Eoff, Din = unpack_params_stage1(params)
        ...
        y_pred = np.abs(Dout @ M @ P @ Din @ E)**2

        # Compute the cost as the sum of squared differences between predicted and true outputs
        return y_pred - y_true

    def unique_residual_stage2(x, y_true, params):
        Dout = unpack_params_stage2(params)
    ...
    def global_residuals_stage1(params):
        res = np.empty(x_space_stage1.shape[0] * 4, dtype=float)
        k = 0
        for x, y_true in zip(x_space_stage1, y_space_stage1):
            r = unique_residual_stage1(x, y_true, params)   # shape (4,)
            res[k:k+4] = r
            k += 4

        Eon, Eoff, Din = unpack_params_stage1(params)
        phys_res = np.array([
            spectral_penalty(Din, target=1.0, weight=100.0, mode="equal"),
        ], dtype=float)

        return np.concatenate([res, phys_res])

    def global_residuals_stage2(params):
        res = np.empty(x_space_stage2.shape[0] * 4, dtype=float)
        k = 0
        for x, y_true in zip(x_space_stage2, y_space_stage2):
            r = unique_residual_stage2(x, y_true, params)   # shape (4,)
            res[k:k+4] = r
            k += 4

        Dout = unpack_params_stage2(params)
        phys_res = np.array([
            spectral_penalty(Dout, target=1.0, weight=100.0, mode="equal"),
        ], dtype=float)
    ...
    x0 = pack_params_stage1(Eon, Eoff, Din)

    result = least_squares(
        fun=global_residuals_stage1,
        x0=x0,
        method="trf",
        loss="soft_l1",
        f_scale=1.0,
    ...
    Eon_fit, Eoff_fit, Din_fit = unpack_params_stage1(result.x)

    x0 = pack_params_stage2(Dout)

    result = least_squares(
        fun=global_residuals_stage2,
        x0=x0,
        method="trf",
        loss="soft_l1",
        f_scale=1.0,
    ...
    Dout_fit = unpack_params_stage2(result.x)
    ...
    def model(active_inputs_tuple, phases_array):
        ...
        return np.abs(Dout_fit @ M @ P @ Din_fit @ E)**2
    ...
    return model, Eon_fit, Eoff_fit, Din_fit, Dout_fit
