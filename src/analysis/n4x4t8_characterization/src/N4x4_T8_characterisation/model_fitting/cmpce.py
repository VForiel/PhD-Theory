...
def fit_CMPCE(scan_data: dict, return_metadata: bool, verbose=False):
    """
    ...
    Cin: ndarray[complex] of shape (4, 4) representing the upstream cross-talk between the inputs of the chip (ideally close to identity but can have small non-zero off-diagonal terms and diagonal terms different from 1 to account for small loss of energy)
    ...
    Cout: ndarray[complex] of shape (4, 4) representing the downstream cross-talk, after the MMI. In practice, due to the matrix degeneracy this matrix also contain imperfections of the MMI itself.
    ...
    # We aim to fit the complex parameters of Eon, Eoff, Cin and Cout, while P is known from the phase shifter settings during the scan.
    ...
    def pack_params(Eon, Eoff, Cin, Cout):
        ...
        Cin_real = np.real(Cin_flat)
        Cin_imag = np.imag(Cin_flat)

        Cout_flat = Cout.flatten()
        Cout_real = np.real(Cout_flat)
        Cout_imag = np.imag(Cout_flat) 
    ...
    def unpack_params(params):
        ...
        Cin_real = params[16:32]
        Cin_imag = params[32:48]
        Cin = Cin_real.reshape(4, 4) + 1j * Cin_imag.reshape(4, 4)

        Cout_real = params[48:64]
        Cout_imag = params[64:80]
        Cout = Cout_real.reshape(4, 4) + 1j * Cout_imag.reshape(4, 4)

        return Eon, Eoff, Cin, Cout

    # Cost function for least squares optimization ----------------------------

    def unique_residual(x, y_true, params):

        Eon, Eoff, Cin, Cout = unpack_params(params)
        ...
        y_pred = np.abs(Cout @ M @ P @ Cin @ E)**2
    ...
    x0 = pack_params(Eon, Eoff, Cin, Cout)
    ...
    Eon_fit, Eoff_fit, Cin_fit, Cout_fit = unpack_params(result.x)
    ...
    def model(active_inputs_tuple, phases_array):
        ...
        return np.abs(Cout_fit @ M @ P @ Cin_fit @ E)**2
    ...
    return model, Eon_fit, Eoff_fit, Cin_fit, Cout_fit
