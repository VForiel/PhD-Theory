...
def fit_RMPRE(scan_data: dict, return_metadata: bool, verbose=False):
    """
    ...
    Rout: ndarray[float] of shape (4, 4) representing the downstream cross-talk, after the MMI. In practice, due to the matrix degeneracy this matrix also contain imperfections of the MMI itself.
    ...
    Rout = np.eye(4, dtype=np.float64)

    # We aim to fit the complex parameters of Eon, Eoff, Rin and Rout, while P is known from the phase shifter settings during the scan.
    ...
    def pack_params(Eon, Eoff, Rin, Rout):
    ...
    def unpack_params(params):
        ...
        Rout = params[32:32+16].reshape((4, 4)) 

        return Eon, Eoff, Rin, Rout

    # Cost function for least squares optimization ----------------------------

    def unique_residual(x, y_true, params):

        Eon, Eoff, Rin, Rout = unpack_params(params)
        ...
        y_pred = np.abs(Rout @ M @ P @ Rin @ E)**2
    ...
    x0 = pack_params(Eon, Eoff, Rin, Rout)
    ...
    Eon_fit, Eoff_fit, Rin_fit, Rout_fit = unpack_params(result.x)
    ...
    def model(active_inputs_tuple, phases_array):
        ...
        return np.abs(Rout_fit @ M @ P @ Rin_fit @ E)**2
