...
def fit_MPE(scan_data: dict, return_metadata: bool, verbose=False):
    ...
    M: ndarray[complex] of shape (4, 4) representing the transfer matrix of the MMI (prior known from the design of the chip)
    ...
    def pack_params(Eon, Eoff, M):
        ...
        M_flat = M.flatten()
        M_real = np.real(M_flat)
        M_imag = np.imag(M_flat)
    ...
    def unpack_params(params):
        ...
        M_real = params[16:32]
        M_imag = params[32:48]
        M = M_real.reshape(4, 4) + 1j * M_imag.reshape(4, 4)
    ...
    def unique_residual(x, y_true, params):
        ...
        Eon, Eoff, M = unpack_params(params)
    ...
    Eon_fit, Eoff_fit, M_fit = unpack_params(result.x)
    ...
    return model, Eon_fit, Eoff_fit, M_fit